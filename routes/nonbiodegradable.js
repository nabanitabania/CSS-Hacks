...
router.get('/item/:id',auth, async (req,res)=>{
        ...
        res.render("nonbiodegradable/showProduct",{getItem,user,infoOfSeller,flashMessages: res.locals.flashMessages})
})
...
router.post('/item/:id', auth , async (req,res) => {
    ...
    else 
    {
      ...
      link3="http://"+req.get('host')+"/profile?id="+user.userId;
      const query = "";
